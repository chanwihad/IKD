...
class ValidatorController extends Controller
{
    ...
    {
        $this->authorize('manage indikator', User::class);
        $this->authorize('validate indikator', User::class);
        if ($this->user->isValidator()) {
    ...
    public function indicatorSave(Request $request)
    {
        ...
        if ($this->user->isValidator()) {
            ...
            if ($request->get('id') == null) {
                ...
                if($indikator){
                    return redirect(route('validatorIndicatorList'))->with('success', 'Berhasil menyimpan data indikator baru');
                }
                return redirect(route('validatorIndicatorList'))->with('warning', 'Gagal menyimpan data indikator baru');
            } else {
                $indikator = Indikator::indikatorSaveUpdate($data, $request->id);
                if($indikator){
                    return redirect(route('validatorIndicatorList'))->with('success', 'Berhasil memperbarui data indikator');
                }
                return redirect(route('validatorIndicatorList'))->with('warning', 'Gagal memperbarui data indikator');
            }
        }
        return abort(403, "Silahkan Login Terlebih Dahulu");
    }
    ...
    public function indicatorDetailSave(Request $request)
    {
        ...
        if ($this->user->isValidator()) {
            ...
            if ($request->get('id') == null) {
                ...
                if($indikator_detail){
                    return redirect(route('validatorIndicatorDetailList', $request->indikator_id))->with('success', 'Berhasil menyimpan data indikator baru');
                }
                return redirect(route('validatorIndicatorDetailList', $request->indikator_id))->with('warning', 'Gagal menyimpan data indikator baru');
            } else {
                $indikator_detail = IndikatorDetail::indikatorDetailSaveUpdate($data, $request->id);
                if($indikator_detail){
                    return redirect(route('validatorIndicatorDetailList', $request->indikator_id))->with('success', 'Berhasil memperbarui data indikator');
                }
                return redirect(route('validatorIndicatorDetailList', $request->indikator_id))->with('warning', 'Gagal memperbarui data indikator');
            }
        }
        return abort(403, "Silahkan Login Terlebih Dahulu");
    }
